...
const handleBorder = ([, direction = "", semanticVal = ""]) =>
  directionMap[direction.substring(1)]?.map((dir) => [
    `border${dir}-color`,
    h.datavizToken(`border${semanticVal}`),
  ]);
...
const getSemanticRegEx = (groupName, directions) =>
  new RegExp(
    `^dv-s-${groupName}${directions ? `(-[${directions}])?` : ""}((-[^\\/]+)?(\\/(0|[1-9][0-9]?|100))?)?$`
  );
...
export const datavizRules = [
  ...
  [
    getSemanticRegEx("fill"),
    ([, semanticVal = ""]) => ({ fill: h.datavizToken(`fill${semanticVal}`) }),
  ],
  [
  ...
  [
    getSemanticRegEx("text"),
    ([, semanticVal = ""]) => ({ color: h.datavizToken(`text${semanticVal}`) }),
  ],
  [
    getSemanticRegEx("icon"),
    ([, semanticVal = ""]) => ({ color: h.datavizToken(`icon${semanticVal}`) }),
  ],
  [
