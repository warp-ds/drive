...
describe('outline', () => {
  ...
  test('supports setting arbitrary outline color variables with alpha channel', async ({ uno }) => {
    const classes = [
      'outline-[--w-black/90]',
      'outline-[--w-s-color-background-positive-selected-hover/100]',
      'outline-[--w-rgb-white/0]',
      'outline-[var(--w-color-text-link-active)/55]',
      'outline-[var(--w-s-color-border)/60]',
      'outline-[--w-s-color-background/5]',
      'outline-[var(--w-black)/100]',
      'outline-[--w-white/5]',
      'outline-[--w-rgb-black/75]',
      'outline-[var(--w-s-rgb-border-disabled)/12]',
    ];
    const { css } = await uno.generate(classes);
    expect(css).toMatchSnapshot();
  });
